# k8s.io/apimachinery - Architecture Overview
...
graph TB
    subgraph "Core Runtime"
        Scheme[Scheme<br/>Type Registry]
        Codec[Codec<br/>Serialization]
        Converter[Converter<br/>Version Conversion]
    end
    
    subgraph "API Metadata"
        Meta[Meta<br/>Object Metadata]
        RESTMapper[RESTMapper<br/>Resource Mapping]
    end
    
    subgraph "Selection & Filtering"
        Labels[Labels<br/>Label Selectors]
        Fields[Fields<br/>Field Selectors]
    end
    ...
    subgraph "Watch & Events"
        Watch[Watch Interface]
        StreamWatcher[Stream Watcher]
    end
    
    subgraph "Utilities"
        Resource[Resource<br/>Quantity]
        Wait[Wait<br/>Polling]
        Sets[Sets<br/>Collections]
        Validation[Validation]
        Patch[Patch<br/>JSON/Strategic]
    end
    ...
    style Codec fill:#e1f5ff
    style Converter fill:#e1f5ff
    style Meta fill:#fff4e1
    style Labels fill:#f0ffe1
    style Fields fill:#f0ffe1
```
...
```mermaid
graph LR
    Object[Object Interface] --> Scheme
    Scheme --> GVK[GroupVersionKind]
    Scheme --> Type[Go Type]
    Scheme --> Converter
    Converter --> InternalVersion[Internal Version]
    InternalVersion --> ExternalVersion[External Version]
...
sequenceDiagram
    ...
    Converter->>Scheme: Convert(internal, external)
    Scheme-->>Converter: Converted Object
    Converter-->>Codec: Versioned Object
    Codec->>Serializer: Encode(object, writer)
    Serializer-->>Codec: Serialized Data
    Codec-->>Client: Encoded Bytes
...
```mermaid
graph TD
    GVK[GroupVersionKind] --> Group[Group<br/>e.g., apps]
    GVK --> Version[Version<br/>e.g., v1]
    GVK --> Kind[Kind<br/>e.g., Deployment]
    
    GVR[GroupVersionResource] --> GroupR[Group<br/>e.g., apps]
    GVR --> VersionR[Version<br/>e.g., v1]
    GVR --> Resource[Resource<br/>e.g., deployments]
    
    RESTMapper[RESTMapper] --> GVK
    RESTMapper --> GVR
    
    style GVK fill:#e1f5ff
    style GVR fill:#fff4e1
    style RESTMapper fill:#f0ffe1
```
...
```mermaid
graph LR
    V1[External v1] --> Internal[Internal Version]
    V2[External v2] --> Internal
    V1Beta1[External v1beta1] --> Internal
